...
public class PostServiceImpl implements PostService {
  ...
  public PostResponse create(PostRequest postRequest) {
    try {
      ...
      if (!principal.getSubject().equals(author.getPhone())) {
        log.error(
            "Unable to create a post, access forbidden attempted by userId: {}",
      ...
      log.info(
          "Post with id {} created successfully for user id {}",
          savedPost.getId(),
          postRequest.getAuthorId());
  ...
  @Override
  public CommunityPostsResponse findAllByCommunity(
      Long communityId, String search, Pageable pageable) {
    try {
      Specification<Post> spec =
          Specification.where(
              (root, query, cb) -> cb.equal(root.get("community").get("id"), communityId));
      ...
      Page<PostResponseByCommunity> posts =
          postRepository
              .findAll(spec, pageable)
              .map(
                  post -> {
  ...
  public Page<PostResponse> findAllByAuthor(Long authorId, String search, Pageable pageable) {
    try {
      Specification<Post> spec =
          Specification.where(
              (root, query, cb) -> cb.equal(root.get("author").get("id"), authorId));
  ...
  public void updatePostTotalComments(Post post) {
    try {
      ...
      log.info(
          "Post with id {} comment count updated to {}", post.getId(), post.getTotalComments());
    } catch (RuntimeException ex) {
