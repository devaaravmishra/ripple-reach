...
public class RoleServiceImpl implements RoleService {
    ...
    public Role createRoleIfNotExists(RoleName roleName) {
        try {
            ...
            log.info("Role with id: {}, name: {} is created or already exists", role.getId(), role.getName());

            return role;
        } catch (RuntimeException ex) {
            log.error("Error while creating or fetching role with name: {}", roleName, ex);
            throw new RippleReachException("Error while creating or fetching role with name: " + roleName);
        }
    }
}
