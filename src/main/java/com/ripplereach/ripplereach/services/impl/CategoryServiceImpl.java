...
public class CategoryServiceImpl implements CategoryService {
    ...
    public Category findById(Long categoryId) {
        return getCategoryById(categoryId).orElseThrow(() -> {
    ...
    public Category update(Long categoryId, String name, String description) {
        ...
        boolean anotherCategoryAlreadyExists = existingCategory.isEmpty() && categoryRepository.existsByName(name);
    ...
    private Optional<Category> getCategoryById(Long categoryId) {
        try {
            ...
            throw new RippleReachException("Error while retrieving category with categoryId: " + categoryId);
        }
    }
    ...
    private void validateCategoryNameAndDescription(String name, String description) {
        if (isFieldEmpty(name) || isFieldEmpty(description)) {
            ...
            throw new HttpClientErrorException(HttpStatus.BAD_REQUEST, "Unable to save category, invalid request!");
        }
    }
    ...
    private Category saveCategory(Category category) {
        try {
            ...
            log.info("Category with categoryId: {}, name: {} saved successfully!", savedCategory.getId(), savedCategory.getName());
            return savedCategory;
        } catch (RuntimeException ex) {
            log.error("Error while saving category with name: {}", category.getName(), ex);
            throw new RippleReachException("Error while saving category with name: " + category.getName());
        }
    }
}
